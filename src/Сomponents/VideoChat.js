...
function VideoChat(props) {
  ...
  // let userName = props.location.user.name !== undefined || props.location.user.name !== "" ? props.location.user.name : "consultant";

  const [socket, setSocket] = useState(props.location.socket)
  const [me, setMe] = useState(props.location.user.me)
  const [stream, setStream] = useState(props.location.stream)
  ...
  // const myVideo = props.location.myVideo
...
console.log(11111);
  ...
  // if (  props.location.user.id !== undefined  ) {
  //   callUser(props.location.user.id);
  // }
  ...
  return (
    <>
      ...
      <button onClick={() => {console.log("3232"); setCount(count + 1)}}>
        Нажми на меня
      </button>
